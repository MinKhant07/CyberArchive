...
const appId = 'my-cyber-archive'; // ကြိုက်တဲ့နာမည်ပေးပါ

// --- Main App Component ---
export default function CyberArchiveApp() {
  ...
  useEffect(() => {
    ...
    // Path: /artifacts/{appId}/users/{userId}/secure_uploads
    const q = query(
      collection(db, 'artifacts', appId, 'users', user.uid, 'secure_uploads')
    );
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        // Sort by timestamp manually since we avoid complex queries
        fetchedItems.sort(
  ...
  const handleFileChange = (e) => {
    ...
    if (selectedFile) {
      ...
      // Limit file to ~700KB to be safe including metadata.
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteDoc(
        doc(db, 'artifacts', appId, 'users', user.uid, 'secure_uploads', id)
      );
    } catch (err) {
      ...
      <div className="relative z-10 container mx-auto p-4 max-w-4xl">
        ...
        <header className="mb-8 border-b-2 border-cyan-800 pb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs md:text-sm text-gray-500 mt-1 uppercase tracking-[0.3em]">
              Secure Data Vault v2.0.77
            </p>
          </div>
            ...
            <div className="bg-[#0a0a0a] border border-cyan-900/50 p-6 relative overflow-hidden group">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-1">
                  <label className="text-xs uppercase text-cyan-700">
                    Identity Name
                  </label>
                  <input
                    type="text"
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <label
                      htmlFor="file-upload"
                      className={`w-full flex items-center justify-center gap-2 p-3 border border-dashed ${
                        fileName
                          ? 'border-green-500 text-green-400'
                          : 'border-gray-700 text-gray-500'
                      } bg-[#0f0f0f] cursor-pointer hover:bg-[#151515] transition-colors text-sm`}
                    >
                      {fileName ? (
                        <>
                ...
                <button
                  type="submit"
                  disabled={uploading}
                  className={`w-full py-2 px-4 mt-4 font-bold text-sm uppercase tracking-wider relative overflow-hidden group ${
                    uploading
                      ? 'bg-gray-800 cursor-not-allowed'
                      : 'bg-cyan-900/30 hover:bg-cyan-800/50 text-cyan-300 border border-cyan-600'
                  }`}
                >
                  ...
                  {!uploading && (
                    <div className="absolute inset-0 h-full w-0 bg-cyan-600/20 group-hover:w-full transition-all duration-300"></div>
                  )}
                </button>
              </form>
            </div>
            ...
            <div className="grid gap-4 max-h-[600px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-cyan-900 scrollbar-track-[#0a0a0a]">
              {items.length === 0 ? (
                ...
                items.map((item) => (
                  <div
                    key={item.id}
                    className="bg-[#080808] border-l-4 border-pink-500 p-4 relative group hover:bg-[#0a0a0a] transition-colors shadow-lg shadow-black/50"
                  >
